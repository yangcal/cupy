...
"""
    ...
    def _minor_index_fancy_sorted(self, idx, M, n_idx, new_shape):
        ...
        # sort_order[k] = j  means idx[j] is the k-th smallest requested val. #
        # ------------------------------------------------------------------ #
        sort_order = cupy.argsort(idx)          # shape (n_idx,), int64
        sorted_idx = idx[sort_order]            # sorted minor-axis requests

        # ------------------------------------------------------------------ #
        # Step 2: for each source entry p, find the range [lo[p], hi[p])      #
        # in sorted_idx whose value equals self.indices[p].                    #
        # cnt[p] = number of output entries generated by source entry p.      #
        # ------------------------------------------------------------------ #
        ...
        # cum_cnt[p+1] = number of output entries from source entries 0..p.   #
        # For each output position q, out_src[q] is the source entry index.   #
        # ------------------------------------------------------------------ #
        ...
        # Output minor index = sort_order[lo[src] + offset] ∈ [0, n_idx).    #
        # ------------------------------------------------------------------ #
        ...
        # ------------------------------------------------------------------ #
        # Step 5: sort output by (major, minor) for canonical (sorted)        #
        # indices within each major group.                                     #
        # ------------------------------------------------------------------ #
        ...
        # Bypass the tuple-3 constructor (check_contents=True would downcast  #
        # int64 indices if values fit in int32).                               #
        # Output minor-axis indices are in [0, n_idx): small values even for  #
        # int64 input, so out_idx_dtype is int32 for typical n_idx.           #
        # ------------------------------------------------------------------ #
